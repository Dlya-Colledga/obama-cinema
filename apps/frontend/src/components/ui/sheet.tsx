...
>(({ side = 'right', className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-lg p-1 text-muted-foreground transition-opacity hover:text-foreground hover:bg-white/5 focus:outline-none focus:ring-2 focus:ring-primary/40 disabled:pointer-events-none">
        <X className="size-5" />
...
const SheetHeader = ({
  ...
  <div
    className={cn(
      'flex flex-col space-y-2 text-left pb-4 border-b border-white/10',
      className
    )}
    {...props}
  />
